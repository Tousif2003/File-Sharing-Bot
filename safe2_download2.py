...
if not multi_clients:
    raise Exception("❌ No clients found in config.env!")
...
async def safe_download(client: Client, message):
    ...
    print(f"📥 Starting download: {file_path} | Chunk: {CHUNK_SIZE//1024} KB | Size: {file_size/1024/1024:.2f} MB")
        ...
        with open(file_path, mode) as f:
            if os.path.exists(file_path):
                ...
                if offset >= file_size:
                    ...
                    return file_path
                f.seek(offset)
                print(f"🔄 Resuming from {offset/1024/1024:.2f} MB")
            ...
            while offset < file_size:
                try:
                    ...
                    print(f"⬇️ {progress:.2f}% | {speed_mb_s:.2f} MB/s | Offset: {offset/1024/1024:.2f} MB", end="\r")

                except asyncio.TimeoutError:
                    retries += 1
                    print(f"\n⚠️ Timeout at {offset}, retry {retries}/{MAX_RETRIES}")
                    if retries >= MAX_RETRIES:
                        print("❌ Aborting due to repeated timeouts")
                        break
                    ...
                    retries += 1
                    print(f"\n❌ RPCError {type(e).__name__}: {e}, retry {retries}/{MAX_RETRIES}")
                    if retries >= MAX_RETRIES:
                        break
                    ...
                    retries += 1
                    print(f"\n⚠️ Unknown error: {e}, retry {retries}/{MAX_RETRIES}")
                    if retries >= MAX_RETRIES:
                        break
                    await asyncio.sleep(3)

    print(f"\n✅ Download finished: {file_name}")
    return file_path
...
async def connection_watchdog(client, name):
    while True:
        ...
        try:
            ...
            except Exception as e: print(f"❌ Failed to restart {name}: {e}")
...
async def main():
    ...
    print("⚡ ThunderBot Multi-File Stable Downloader started")
    print("✅ Concurrent download ready at 5–10 MB/s per file, up to 5 files at a time")
